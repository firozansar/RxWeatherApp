...
class WeatherCitySearchActivity : AppCompatActivity() {
    ...
    private fun setupSearchedCityClickedListener() {
        binding.cityButton.setOnClickListener {
            if (!isConnectedToInternet) {
                Toast.makeText(
                    this,
                    getString(R.string.user_has_not_internet_connection_message),
                    Toast.LENGTH_SHORT
                ).show()
            } else {
                processRequestStartUI()
                val searchedCityName = binding.autocompleteTextView.text.toString()
                setupWeatherDetailsObserver(searchedCityName)?.let { it ->
            ...
            .subscribe(
                ...
                { throwable: Throwable? ->
                    ...
                    if (!isConnectedToInternet) {
                        ...
                        Toast.makeText(
                            this,
                            getString(R.string.error_with_fetching_weather_details_message),
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                }
    ...
    private fun navigateToDetailsActivity(weatherResponse: WeatherDetailsDTO?) {
        ...
        intent.putExtra(
            getString(R.string.intentWeatherDetailsParcelerBundleName),
            Parcels.wrap(weatherResponse)
        )
                ...
                { isConnected: Boolean? ->
                    isConnected?.let {
                        if (!isConnected)
                            Toast.makeText(
                                this,
                                getString(R.string.user_has_lost_internet_connection_message),
                                Toast.LENGTH_SHORT
                            ).show()
