...
class WeeklyWeatherAdapter(val list: ArrayList<WeeklyWeatherDTO>) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.textViewDayMaxTemp.text = convertToValueWithUnit(
            2,
            unitDegreesCelsius,
            convertFahrenheitToCelsius(item.maxTemp.toDouble())
        )
        holder.textViewDayMinTemp.text = convertToValueWithUnit(
            2,
            unitDegreesCelsius,
            convertFahrenheitToCelsius(item.minTemp.toDouble())
        )
